...
// resize width of screen, check for labels of chart
...
function paint() {
    const dataProstasiaPliromi = calculateOikogeneiakiProstasi();
    // console.log(dataProstasiaPliromi)
    // console.log($(".canvas")[0]);
    if (dataProstasiaPliromi === 'empty') {
        $(".canvas").empty();
        $(".canvas")[0].style.backgroundColor = 'skyblue';
        return;
    }
    // console.log("1");
    $(".canvas")[0].style.backgroundColor = 'rgb(236,205,193)';
    $(".canvas").empty();
    $(".canvas").append(`<canvas class="my-4 w-100" id="myChart"></canvas>`)
    // console.log("2");
    const dataEpendytiko = ending_total_value(
        (Number($("#arxikoposo").val()) / 12),
        Number($("#efapaj").val()),
        Number($("#epitokio").val()),
        Number($("#ethsiaanaprosarmogh").val()),
        Number($("#diarkeiae").val()),
        true);
    const dataEpendytikoPliromi = total_money_paid_for_unit_link(
        (Number($("#arxikoposo").val()) / 12),
        Number($("#efapaj").val()),
        Number($("#epitokio").val()),
        Number($("#ethsiaanaprosarmogh").val()),
    ...
    const pliromi = [];
    // console.log("3");
    for (let i = 0; i < dataEpendytikoPliromi.length; i++) {
        if (i < dataProstasiaPliromi.length) {
            pliromi.push(Math.round(dataProstasiaPliromi[i] + dataEpendytikoPliromi[i]))
        } else {
            pliromi.push(Math.round(dataEpendytikoPliromi[i] + dataProstasiaPliromi[dataProstasiaPliromi.length - 1]))
        }
    }
    // console.log("4");
    const efapax = Number($(kataballomenoefapajseperiptwshapobiwshs).val());
    let dataEfapax = [];
    const yearsProstasia = Number($("#diarkeiap").val());
    let chosenCover1 = $("#kalyjh option:selected").text();
    if (chosenCover1 == "TT05") {
        for (let cc = 0; cc < yearsProstasia; cc++) {
            dataEfapax.push(efapax);
        }
    } else if (chosenCover1 == "TT06") {
        dataEfapax = calculateOikogeneiakiProstasi(true);
        console.log(dataEfapax, "Prasini grammi");
    }

    var ctx = document.getElementById('myChart').getContext('2d');
    // console.log("5");
    const years = Number($("#diarkeiae").val());

    const label = [];
    // console.log("6");
    for (let cc = 0; cc < years; cc++) {
        label.push([cc + 1]);
    }
    console.log("7");
    const synolikiProstasia = [];
    for (let counter = 0; counter < dataEpendytiko.length; counter++) {
        if (counter < dataEfapax.length) {
            synolikiProstasia.push(dataEpendytiko[counter] + dataEfapax[counter]);
        } else {
            synolikiProstasia.push(dataEpendytiko[counter]);
        }
    }
    console.log(synolikiProstasia);
    ...
    const obj = {
        ...
        data: {
            ...
            datasets: [
                {
                    label: "Συνολική Προστασία",
                    data: synolikiProstasia,
                    borderColor: "rgb(128,0,0)",
                    borderWidth: 2,
                },
                {
                    label: "Επενδυτικό Πρόγραμμα",
                    data: dataEpendytiko,
                    borderColor: 'rgb(68,102,201)',
                    borderWidth: 2,
                },
                {
                    label: "Εξασφάλιση",
                ...
                {
                    label: "Συνολικές Καταβολές",
                    data: pliromi,
                    borderColor: ' rgb(132,132,130)',
                    borderWidth: 2
                }]
        },
        options: {
            ...
            fillColor: 'rgba(241,241,241,0.8)',
            scales: {
                yAxes: [{
                    display: true,
                    ticks: {
                        max: Math.trunc(1.1 * Math.round(Math.max(
                            ...dataEpendytiko,
                            efapax,
                            ...pliromi,
                            ...synolikiProstasia) / 1000) * 1000),    // minimum will be 0, unless there is a lower value.
                        // OR //
